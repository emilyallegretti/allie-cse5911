...
- Test folder containing Pytest unit tests. Each component being tested is separated into a separate file under this folder and distinguishable by filename.
...
- **Output of Active Participation Indicator Analysis**: All of the indicators in the above graph are analyzed in the code but not all of them are shown in the output (either on the graph or as an annotation on the graph). The next goal of this project would be to have a more comprehensive output of all the active participation indicators. For example. emoji scores, time spent on announcement page, time spent reading slides, etc. These indicators can be found in their respective classes: EmojiSelectSequence, MouseEnterSlides+MouseLeaveSlides, etc.
- **States**: Not every Event type has been implemented into an EventContainer or State/StateSequence class. For example, the states of reading slides, being on the announcements page, and being on the emoji board page are not yet implemented but would be necessary in order to get a functional graphical output showing all indicators as described above. Before moving on to a possible ML implementation, we recommend implementing State/StateSequence/EventContainer classes for each of the indicators in the table above.
- **Testing**: Test suite should be updated to test functionality of all State and StateContainer class models. Tests should be implemented for the rest of the indicator analyses from the table above: announcements, emoji board, reading slides, etc. Only limited amount of indicators are being tested as of now; check Tests folder for specific categories. UI/regression testing for Plotly output would be beneficial as well.
